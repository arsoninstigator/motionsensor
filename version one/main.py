...
alarm_song = ["E5", "G5", "A5", "P", "E5", "G5", "B5", "A5", "P", "E5", "G5", "A5", "P", "G5", "E5"]
...
def play_alarm():
    for note in alarm_song:
        if note == "P":
            be_quiet()
        else:
            play_tone(tones[note])
        sleep(0.3)
    be_quiet()

# Continuously monitor PIR sensor and control LED
while True:
    if pir_pin.value() == 1:
        print("PIR value: 1    Status: Motion detected!")
        led_pin.on()
        play_alarm()
    else:
        print("PIR value: 0    Status: Waiting for movement...")
        led_pin.off()
    sleep(1)
